...
<meta name="theme-color" content="#080812"><script>(function(){try{const t=localStorage.getItem('beyond-tv-theme');document.documentElement.dataset.tvTheme=['dark','light','sunset'].includes(t)?t:'dark';}catch(e){document.documentElement.dataset.tvTheme='dark';}})();</script>
...
<section class="classic-shell" aria-label="Choose a live channel">
  <div class="channel-switcher">
    ...
    <button type="button" class="channel-switch" data-tv-channel="preschool" data-player-url="https://www.youtube-nocookie.com/embed/61fSXCbzF1M?autoplay=1&amp;mute=1&amp;playsinline=1&amp;enablejsapi=1" data-title="Preschool TV" data-current="Bluey Full Episodes" data-lineup="Bluey · Bubble Guppies · Preschool learning" data-next="Bubble Guppies" data-open="https://beyondimagination.co.technology/beyond-tv/channel.php?slug=bubble-guppies"><span><strong>🐾 Channel 3 · Preschool TV</strong><small>Bluey · Bubble Guppies · Preschool</small></span><span class="live-dot">● PLAY</span></button>
...
<script>(function(){const root=document.documentElement,btn=document.querySelector('[data-tv-theme-toggle]'),themes=['dark','light','sunset'];if(!btn)return;function apply(t){if(!themes.includes(t))t='dark';root.dataset.tvTheme=t;const next=themes[(themes.indexOf(t)+1)%themes.length],icons={dark:'🌙',light:'☀️',sunset:'🌅'},labels={dark:'Dark',light:'Light',sunset:'Sunset'};btn.innerHTML=icons[t]+' <span>'+labels[t]+'</span>';btn.setAttribute('aria-label','Current theme '+labels[t]+'. Switch to '+labels[next]+' theme');btn.title='Switch to '+labels[next]+' theme';document.querySelector('meta[name=theme-color]')?.setAttribute('content',t==='light'?'#f4f6fb':t==='sunset'?'#32133f':'#080812');}apply(root.dataset.tvTheme||'dark');btn.addEventListener('click',()=>{const current=themes.includes(root.dataset.tvTheme)?root.dataset.tvTheme:'dark',next=themes[(themes.indexOf(current)+1)%themes.length];try{localStorage.setItem('beyond-tv-theme',next)}catch(e){}apply(next)});})();</script></body>
</html>
